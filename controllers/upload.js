...
const multerConfig = multer.diskStorage({
    ...
    filename: async (req, file, callback)=>{
        try{
            ...
            for await (subscriber of subscribers){
                await Notification.build({media_id:new_media.id,user_id:subscriber.id}).save();
                const message = {
                    from: "frostwolfatris@gmail.com",
                    to: subscriber.email,
                    subject: 'Account recovery',
                    text: 'Visit this to recover account : \n' +
                        ` http://localhost:3000/menu/watch/${new_media.index}`
                }
